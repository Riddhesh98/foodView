...
const UserLogin = () => {
  ...
  const handleSubmit =  async(e)=>{
    ...
    const response = await axios.post(`${import.meta.env.VITE_BASE_URL}/api/auth/users/login`, data,
    {
  ...
  return (
    <div className="min-h-dvh flex items-center justify-center px-4 py-8 bg-fixed bg-[radial-gradient(circle_at_30%_20%,#f1f5f9_0%,#f9fafb_60%)]">
      <div className="w-full max-w-[420px] bg-white border border-slate-200 p-6 pb-5 rounded-lg shadow-md flex flex-col gap-5" role="region" aria-labelledby="user-login-title">
        <header className="text-center">
          <h1 id="user-login-title" className="text-[1.45rem] font-semibold tracking-[.5px] mb-2">Welcome back</h1>
          <p className="text-sm text-slate-600">Sign in to continue your food journey.</p>
        </header>
        ...
        <form className="flex flex-col gap-4" onSubmit={handleSubmit} noValidate>
          ...
          <div className="flex flex-col gap-1">
            ...
            className="appearance-none w-full min-w-0 border border-slate-200 bg-slate-100 px-3 py-2.5 rounded outline-none text-slate-900 transition-colors focus-visible:border-blue-600 focus-visible:ring-2 focus-visible:ring-blue-600/40 focus-visible:bg-white" id="password" name="password" type="password" placeholder="••••••••" autoComplete="current-password" />
          </div>
        ...
        <div className="text-center text-sm text-slate-600">
          New here? <a className="font-semibold text-blue-600 hover:text-blue-700" href="/user/register">Create account</a>
        </div>
      </div>
    </div>
  );
